...
if __name__ == '__main__':
    ...
    land_present = read_EASE2_grid_land_mask(resolution=3,pole='north',convert_ice=True)

    lats_25,lons_25 = read_EASE2_grid_locations(resolution=25,pole='north')
    ...
    for ix in range(0,720):
        ...
        if os.name == 'nt':
            nc_file = f'L:/access/land_water/land_fraction_1440_721_{footprint_diameter_km}km.from_nsidc_3km_mask.north.ease25.v5.nc'
        elif  os.name == 'posix':
            nc_file = f'/mnt/ops1p-ren/l/access/land_water/land_fraction_1440_721_{footprint_diameter_km}km.from_nsidc_3km_mask.north.ease25.v5.nc'
        else:
            raise ValueError
        
        encoding = {"land_fraction":{'zlib' : True, 'complevel': 4 }}
        land_mask_DS.to_netcdf(nc_file,encoding=encoding)   
        print(f'Finished Row = {ix}') 
